...
const f_toggle = document.querySelector('.biz_info > p');
...
list_toon_item.forEach((p_el, p_index)=>{
    ...
    let c_items_ul_queryAll = p_el.querySelectorAll('ul');
    let c_items_li = c_items_ul.querySelectorAll('li');
    let c_items_li_queryAll = c_items_ul.querySelectorAll('li');
    ...
    if(c_items_ul.classList.contains('list_toon') || c_items_ul.classList.contains('tab_list')){
        ...
        if(c_items_ul.classList.contains('list_toon')){
            c_items_ul.style.width = c_items_li_width * c_items_li.length+'px'; //부모에 width 지정해줍니다.
        }else{
            
            c_items_ul.style.width = (c_items_li_width - 6) * c_items_li.length+'px'; //부모에 width 지정해줍니다.
        }
        ...
        c_items_ul_parent.addEventListener('touchend', (event)=>{
            isDown = false;
            let maxScroll = 0;
            if (c_items_ul.offsetWidth > c_items_ul_parent.offsetWidth) {
                maxScroll = c_items_ul_parent.offsetWidth - c_items_ul.offsetWidth;
            }
            if (xScroll_length < 0 && xScroll_length < maxScroll) {
                xScroll_length = maxScroll;
            } else if (xScroll_length > 0) {
                xScroll_length = 0;
            } else if (xScroll_length < 0 && xScroll_length > maxScroll) {
                xScroll_length = xScroll_length;
            }
            c_items_ul.style.transition = 'transform 0.45s cubic-bezier(0.1, 0.57, 0.1, 1)';
            c_items_ul.style.transform = `translate(${xScroll_length}px, 0)`;
        });
    }else if(c_items_ul.classList.contains('list_toon_st2')){
        let swipe_inner = p_el.querySelector('.swipe_inner');

        swipe_inner.style.width = (window.innerWidth - 60) * c_items_li.length+'px';

        c_items_ul_queryAll.forEach((el, index)=>{
